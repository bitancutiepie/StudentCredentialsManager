...
async function loadFlashcards() {
    ...
    container.innerHTML = '<div class="loader">Loading flashcards...</div>';

    // Fetch all flashcards from Supabase
    const { data, error } = await db.from('flashcards').select('*').order('created_at', { ascending: false });

    if (error) {
        console.error('Error loading flashcards:', error);
        container.innerHTML = '<p style="text-align: center; color: #d63031;">Error loading flashcards.</p>';
        return;
    }

    // Auto-delete expired cards (older than 7 days)
    const now = new Date();
    const active = [];
    const expiredIds = [];
...
function renderFlashcardCards(cards) {
    const container = document.getElementById('flashcard-list');
    if (!container) return;
            ...
            <div class="class-card" style="border-left: 5px solid ${color}; position: relative; animation-delay: ${i * 0.08}s;">
                ...
                <div style="display: flex; align-items: flex-start; gap: 15px; flex-wrap: wrap;">
                    ...
                    <div style="flex: 1; min-width: 180px;">
                        ...
                        ${card.description ? `<p style="margin: 0 0 8px 0; font-family: 'Patrick Hand'; font-size: 1rem; color: #636e72; line-height: 1.4;">${escapeHTML(card.description)}</p>` : ''}
                        <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                            ...
                            <small style="color: #aaa; font-family: 'Patrick Hand'; font-size: 0.85rem;">
                                <i class="fas fa-clock"></i> ${addedAt}
                            </small>
...
window.addFlashcard = async function (e) {
    ...
    const { error } = await db.from('flashcards').insert([{ topic, link, description }]);
    if (error) {
        console.error('Error adding flashcard:', error);
        showToast('Error saving flashcard: ' + error.message);
        return;
    }
...
window.deleteFlashcard = async function (id) {
    ...
    if (error) {
        showToast('Error deleting flashcard: ' + error.message);
        return;
    }
